...
struct RamImageWriteEvent {
    uint32_t pc = 0;
    uint32_t addr = 0;
    uint32_t width = 0;
};
RamImageWriteEvent ram_image_write_events[8]{};
unsigned ram_image_write_event_count = 0;

void record_ram_image_write(uint64_t, uint32_t pc, uint8_t,
                            uint32_t addr, uint32_t width) {
    if (ram_image_write_event_count >= 8u) return;
    ram_image_write_events[ram_image_write_event_count++] = {pc, addr, width};
}
...
void test_waitcnt_updates_game_pak_cycles() {
    gba::GbaBus bus;

    // Reset WAITCNT: WS0 N16=4, S16=2; 32-bit combines N+1+S / 2S+1.
    check_eq("waitcnt_cycles", "reset_n16",
             bus.access_cycles(0x08000000u, 2, false), 5u);
    check_eq("waitcnt_cycles", "reset_s16",
             bus.access_cycles(0x08000000u, 2, true), 3u);
    check_eq("waitcnt_cycles", "reset_n32",
             bus.access_cycles(0x08000000u, 4, false), 8u);
    check_eq("waitcnt_cycles", "reset_s32",
             bus.access_cycles(0x08000000u, 4, true), 6u);

    // Golden Sun's observed early-boot write: WS0 N16=3, S16=1,
    // prefetch enabled. Prefetch state is separate; the wait tables change
    // immediately when the MMIO write commits.
    bus.write16(0x04000204u, 0x4014u);
    check_eq("waitcnt_cycles", "configured_n16",
             bus.access_cycles(0x08000000u, 2, false), 4u);
    check_eq("waitcnt_cycles", "configured_s16",
             bus.access_cycles(0x08000000u, 2, true), 2u);
    check_eq("waitcnt_cycles", "configured_n32",
             bus.access_cycles(0x08000000u, 4, false), 6u);
    check_eq("waitcnt_cycles", "configured_s32",
             bus.access_cycles(0x08000000u, 4, true), 4u);

    // The write above enables prefetch before GBAStore16 calls
    // GBAMemoryStall. At the observed cart PC, mGBA turns the raw one-cycle IO
    // access into -2 cycles; the instruction's other terms keep its total
    // positive. Preserve the signed intermediate exactly.
    check_eq("waitcnt_cycles", "same_store_prefetch_stall",
             bus.data_access_cycles(0x04000204u, 2, false, 0x08002E1Cu),
             -2);
}

struct EwramObserverEvent {
    uint32_t addr = 0;
    uint32_t width = 0;
};
EwramObserverEvent ewram_observer_events[8]{};
unsigned ewram_observer_event_count = 0;

void record_ewram_write(uint32_t addr, uint32_t width) {
    if (ewram_observer_event_count >= 8u) return;
    ewram_observer_events[ewram_observer_event_count++] = {addr, width};
}

void test_fast_ewram_write_observer() {
    gba::GbaBus bus;
    gbarecomp::set_active_bus(&bus);
    ewram_observer_event_count = 0;
    for (auto& event : ewram_observer_events) event = {};

    // Install the same observer at both write seams. The inline path must
    // report EWRAM once, IWRAM not at all, and the slow path once without a
    // second fast-path notification.
    g_runtime_fast_ewram_write_observer = &record_ewram_write;
    gba::g_ws_ewram_write_observer = &record_ewram_write;
    bus_write_u8(0x02000100u, 0x12u);
    bus_write_u16(0x02000102u, 0x3456u);
    bus_write_u32(0x02000104u, 0x789ABCDEu);
    bus_write_u16(0x03000100u, 0xBEEFu);

    // Force only EWRAM through the existing slow bridge. Its observer must
    // remain the sole notification for this store.
    g_fast_ewram = nullptr;
    bus_write_u32(0x02000108u, 0x11223344u);

    check_eq("fast_ewram_observer", "event_count",
             ewram_observer_event_count, 4u);
    check_eq("fast_ewram_observer", "first_addr",
             ewram_observer_events[0].addr, 0x02000100u);
    check_eq("fast_ewram_observer", "first_width",
             ewram_observer_events[0].width, 1u);
    check_eq("fast_ewram_observer", "second_width",
             ewram_observer_events[1].width, 2u);
    check_eq("fast_ewram_observer", "third_width",
             ewram_observer_events[2].width, 4u);
    check_eq("fast_ewram_observer", "slow_addr",
             ewram_observer_events[3].addr, 0x02000108u);
    check_eq("fast_ewram_observer", "slow_width",
             ewram_observer_events[3].width, 4u);
    check_eq("fast_ewram_observer", "fast_u8_value",
             bus.read8(0x02000100u), 0x12u);
    check_eq("fast_ewram_observer", "fast_u16_value",
             bus.read16(0x02000102u), 0x3456u);
    check_eq("fast_ewram_observer", "fast_u32_value",
             bus.read32(0x02000104u), 0x789ABCDEu);
    check_eq("fast_ewram_observer", "slow_u32_value",
             bus.read32(0x02000108u), 0x11223344u);
...
void test_ram_image_common_bus_writer() {
    gba::GbaBus bus;
    ram_image_write_event_count = 0;
    g_runtime_ram_image_write_probe = &record_ram_image_write;
    g_cpu.R[15] = 0x03000198u;
    g_cpu.cpsr = 0u;

    // GbaBus is the direct/interpreter-side write path. Clip overlap exactly
    // as the generated fast path does, and never expose the stored value.
    bus.write16(0x03005B0Bu, 0xAABBu);
    bus.write32(0x03005D58u, 0x11223344u);
    bus.write32(0x02000000u, 0x55667788u);

    check_eq("ram_image_bus_writer", "event_count",
             ram_image_write_event_count, 2u);
    check_eq("ram_image_bus_writer", "first_pc",
             ram_image_write_events[0].pc, 0x03000198u);
    check_eq("ram_image_bus_writer", "first_addr",
             ram_image_write_events[0].addr, 0x03005B0Cu);
    check_eq("ram_image_bus_writer", "first_width",
             ram_image_write_events[0].width, 1u);
    check_eq("ram_image_bus_writer", "second_addr",
             ram_image_write_events[1].addr, 0x03005D58u);
...
int main() {
    ...
    test_waitcnt_updates_game_pak_cycles();
    test_fast_ewram_write_observer();
    test_ram_image_common_bus_writer();
    if (failures) {
        std::printf("\n%d failure(s)\n", failures);
        return 1;
    }
