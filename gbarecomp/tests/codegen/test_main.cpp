...
// Regression for CRASH-01c: shared-trampoline BL sites all resolve to the
// same nested-dispatch return PC (see runtime_call_cancel_return's comment
// in runtime_arm.cpp), so by the time a call site's C-level cancel check
// runs, its frame can be buried under frames pushed while that nested PC
// ran. A top-only cancel_return leaves such frames permanently stranded;
// this must scan to the floor and truncate the matched slot instead.
bool run_call_cancel_return_scan_cases() {
    runtime_init(nullptr);

    // (a) push A, push B, cancel A -> depth returns to before-A and B is
    // discarded (it is provably dead: control already unwound past it).
    runtime_call_push_return(0x00001000u);
    runtime_call_push_return(0x00002000u);
    runtime_call_cancel_return(0x00001000u);
    if (runtime_call_stack_depth() != 0u) {
        std::printf("FAIL runtime_call_cancel_return: buried-frame cancel "
                    "did not truncate to the matched (older) slot, "
                    "depth=%u\n", runtime_call_stack_depth());
        runtime_shutdown();
        return false;
    }

    // (b) cancel with no match -> no-op, depth unchanged.
    runtime_call_push_return(0x00003000u);
    runtime_call_push_return(0x00004000u);
    runtime_call_cancel_return(0x00005000u);
    if (runtime_call_stack_depth() != 2u) {
        std::printf("FAIL runtime_call_cancel_return: unmatched cancel "
                    "changed depth to %u (expected 2)\n",
                    runtime_call_stack_depth());
        runtime_shutdown();
        return false;
    }
    runtime_call_stack_restore(nullptr, 0u);

    // (c) cancel never truncates below the active IRQ-nesting floor. Frame
    // 0x6000 is the "interrupted mainline" frame the floor protects (its
    // slot sits below the floor raised right after it was pushed); a
    // cancel naming it must no-op even though it is a real, older frame.
    runtime_call_push_return(0x00006000u);
    runtime_call_stack_set_floor_for_test(runtime_call_stack_depth());
    runtime_call_push_return(0x00007000u);
    runtime_call_cancel_return(0x00006000u);
    if (runtime_call_stack_depth() != 2u) {
        std::printf("FAIL runtime_call_cancel_return: cancel reached below "
                    "the IRQ floor, depth=%u (expected 2)\n",
                    runtime_call_stack_depth());
        runtime_call_stack_set_floor_for_test(0u);
        runtime_shutdown();
        return false;
    }
    // A frame at/above the floor is still cancellable.
    runtime_call_cancel_return(0x00007000u);
    if (runtime_call_stack_depth() != 1u) {
        std::printf("FAIL runtime_call_cancel_return: at-floor cancel "
                    "did not truncate, depth=%u (expected 1)\n",
                    runtime_call_stack_depth());
        runtime_call_stack_set_floor_for_test(0u);
        runtime_shutdown();
        return false;
    ...
    // (d) plain top-of-stack cancel still behaves as before.
    runtime_call_push_return(0x00008000u);
    runtime_call_push_return(0x00009000u);
    runtime_call_cancel_return(0x00009000u);
    if (runtime_call_stack_depth() != 1u) {
        std::printf("FAIL runtime_call_cancel_return: plain top-of-stack "
                    "cancel left depth=%u (expected 1)\n",
                    runtime_call_stack_depth());
        runtime_shutdown();
        return false;
    }
...
int thumb_literal_test_override(uint32_t pc, uint32_t original,
                                uint32_t* out) {
    if (pc == 0x120u && original == 0x13572468u) {
        *out = 0xCAFEBABEu;
        return 1;
    }
    return 0;
...
bool g_conditional_branch_force = false;
uint32_t g_conditional_branch_last_pc = 0;
uint32_t g_conditional_branch_last_original = 0;

int conditional_branch_test_override(uint32_t pc, uint32_t original,
                                     uint32_t* out) {
    g_conditional_branch_last_pc = pc;
    g_conditional_branch_last_original = original;
    if (pc != 0x100u) return 0;
    *out = g_conditional_branch_force ? 1u : 0u;
    return 1;
}
...
bool run_thumb_alu_immediate_override_cases() {
    ...
    std::size_t literal_idx = kTestCasesCount;
    for (std::size_t i = 0; i < kTestCasesCount; ++i) {
        if (std::strcmp(kTestCases[i].name,
            ...
            arm_cmp_idx = i;
                               ...
                               "thumb_ldr_pc_rel_override_fixture") == 0) {
            literal_idx = i;
        }
    }
    if (sub_idx == kTestCasesCount || add_idx == kTestCasesCount ||
        unconfigured_idx == kTestCasesCount ||
        arm_cmp_idx == kTestCasesCount || literal_idx == kTestCasesCount) {
    ...
    // A reviewed THUMB PC-relative literal preserves the bus read shape but
    // can replace only its loaded value through the separate literal hook.
    codegen_test::bus_reset(0u, 64u * 1024u);
    codegen_test::bus_write_u32_direct(0x0124u, 0x13572468u);
    std::memset(&g_cpu, 0, sizeof(g_cpu));
    g_cpu.cpsr = (1u << 5) | 0x1Fu;
    g_runtime_thumb_literal_override = thumb_literal_test_override;
    kTestFns[literal_idx]();
    if (g_cpu.R[0] != 0xCAFEBABEu) {
        std::printf("FAIL thumb_literal_override: r0=0x%08X\n",
                    g_cpu.R[0]);
...
bool run_conditional_branch_override_cases() {
    std::size_t taken_idx = kTestCasesCount;
    std::size_t not_taken_idx = kTestCasesCount;
    for (std::size_t i = 0; i < kTestCasesCount; ++i) {
        if (std::strcmp(kTestCases[i].name, "thumb_beq_taken") == 0) {
            taken_idx = i;
        } else if (std::strcmp(kTestCases[i].name,
                               "thumb_beq_not_taken") == 0) {
            not_taken_idx = i;
        }
    }
    if (taken_idx == kTestCasesCount || not_taken_idx == kTestCasesCount) {
        std::printf("FAIL conditional_branch_override: fixture missing\n");
        return false;
    }

    // A null handler is the faithful path: the original taken decision and
    // target remain unchanged.
    codegen_test::bus_reset(0u, 64u * 1024u);
    std::memset(&g_cpu, 0, sizeof(g_cpu));
    g_cpu.cpsr = (1u << 5) | (1u << 30) | 0x1Fu;
    g_runtime_conditional_branch_override = nullptr;
    kTestFns[taken_idx]();
    if (!codegen_test::g_dispatch_called ||
        codegen_test::g_last_dispatch_target != 0x106u) {
        std::printf("FAIL conditional_branch_override: null path target=0x%08X\n",
                    codegen_test::g_last_dispatch_target);
        return false;
    }

    // An accepting callback may force a normally-not-taken branch to run.
    codegen_test::bus_reset(0u, 64u * 1024u);
    std::memset(&g_cpu, 0, sizeof(g_cpu));
    g_cpu.cpsr = (1u << 5) | 0x1Fu;
    g_conditional_branch_force = true;
    g_conditional_branch_last_pc = 0;
    g_conditional_branch_last_original = 1;
    g_runtime_conditional_branch_override = conditional_branch_test_override;
    kTestFns[not_taken_idx]();
    if (g_conditional_branch_last_pc != 0x100u ||
        g_conditional_branch_last_original != 0u ||
        !codegen_test::g_dispatch_called ||
        codegen_test::g_last_dispatch_target != 0x106u) {
        std::printf("FAIL conditional_branch_override: forced taken pc=0x%08X "
                    "original=%u target=0x%08X\n",
                    g_conditional_branch_last_pc,
                    g_conditional_branch_last_original,
                    codegen_test::g_last_dispatch_target);
        g_runtime_conditional_branch_override = nullptr;
        return false;
    }

    // The same callback can force a normally-taken branch not to run.
    codegen_test::bus_reset(0u, 64u * 1024u);
    std::memset(&g_cpu, 0, sizeof(g_cpu));
    g_cpu.cpsr = (1u << 5) | (1u << 30) | 0x1Fu;
    g_conditional_branch_force = false;
    g_conditional_branch_last_pc = 0;
    g_conditional_branch_last_original = 0;
    kTestFns[taken_idx]();
    if (g_conditional_branch_last_pc != 0x100u ||
        g_conditional_branch_last_original != 1u ||
        codegen_test::g_dispatch_called || g_cpu.R[15] != 0x102u) {
        std::printf("FAIL conditional_branch_override: forced not-taken pc=0x%08X "
                    "original=%u pc=0x%08X\n",
                    g_conditional_branch_last_pc,
                    g_conditional_branch_last_original, g_cpu.R[15]);
        g_runtime_conditional_branch_override = nullptr;
        return false;
    }

    // Unknown PCs and a rejecting callback preserve the caller's decision.
    if (runtime_conditional_branch(0x104u, 1u) != 1u ||
        runtime_conditional_branch(0x104u, 0u) != 0u) {
        std::printf("FAIL conditional_branch_override: unknown PC changed "
                    "original decision\n");
        g_runtime_conditional_branch_override = nullptr;
        return false;
    }
...
bool run_infinite_cheat_cases() {
    constexpr uint32_t kBase = 0x02000534u;
    constexpr uint32_t kStride = 0x14Cu;
    codegen_test::bus_reset(0x02000000u, 0x1000u);
    runtime_set_infinite_hp(0);
    runtime_set_infinite_pp(0);

    for (unsigned slot = 0; slot < 4u; ++slot) {
        const uint32_t p = kBase + slot * kStride;
        const uint16_t max_hp = static_cast<uint16_t>(100u + slot);
        const uint16_t max_pp = static_cast<uint16_t>(40u + slot);
        const uint16_t cur_hp = static_cast<uint16_t>(20u + slot);
        const uint16_t cur_pp = static_cast<uint16_t>(5u + slot);
        codegen_test::bus_write_u32_direct(
            p, static_cast<uint32_t>(max_hp) |
               (static_cast<uint32_t>(max_pp) << 16));
        codegen_test::bus_write_u32_direct(
            p + 4u, static_cast<uint32_t>(cur_hp) |
               (static_cast<uint32_t>(cur_pp) << 16));
    }

    runtime_apply_infinite_hp_pp();
    if (bus_read_u16_slow(kBase + 4u) != 20u ||
        bus_read_u16_slow(kBase + 6u) != 5u) {
        std::printf("FAIL infinite_cheats: default-off changed party\n");
        return false;
    }

    runtime_set_infinite_hp(1);
    runtime_apply_infinite_hp_pp();
    if (bus_read_u16_slow(kBase + 4u) != 100u ||
        bus_read_u16_slow(kBase + 6u) != 5u) {
        std::printf("FAIL infinite_cheats: HP/PP isolation\n");
        return false;
    }

    runtime_set_infinite_pp(1);
    // A zero maximum is an empty slot and must not turn into a nonzero value.
    const uint32_t empty = kBase + 3u * kStride;
    codegen_test::bus_write_u32_direct(empty, 0u);
    codegen_test::bus_write_u32_direct(empty + 4u, 0x00090008u);
    runtime_apply_infinite_hp_pp();
    if (bus_read_u16_slow(kBase + 6u) != 40u ||
        bus_read_u16_slow(empty + 4u) != 8u ||
        bus_read_u16_slow(empty + 6u) != 9u) {
        std::printf("FAIL infinite_cheats: PP/empty-slot handling\n");
        return false;
    }
    runtime_set_infinite_hp(0);
    runtime_set_infinite_pp(0);
...
int synthetic_mem_write_override(uint32_t pc, uint32_t addr,
                                 uint32_t requested, uint32_t width,
                                 uint32_t* out_value) {
    constexpr uint32_t kPc = 0x08001000u;
    constexpr uint32_t kAddr = 0x02000100u;
    if (!out_value || pc != kPc || addr != kAddr || width != 4u) return 0;
    const uint32_t before = bus_read_u32(addr);
    const uint32_t delta = requested - before;
    *out_value = before + delta + delta;
    return 1;
}

bool run_player_speed_cheat_cases() {
    constexpr uint32_t kWriterPc = 0x08001000u;
    constexpr uint32_t kAccumulator = 0x02000100u;
    codegen_test::bus_reset(kAccumulator, 0x100u);
    g_runtime_mem_write_override = synthetic_mem_write_override;
    runtime_set_mem_write_override_enabled(0);

    // The generic seam is inert while disabled.
    codegen_test::bus_write_u32_direct(kAccumulator, 100u);
    runtime_trace_event(RUNTIME_TRACE_MEM_WRITE, kWriterPc, kAccumulator,
                        112u, 4u);
    bus_write_u32(kAccumulator, 112u);
    if (codegen_test::bus_read_u32_direct(kAccumulator) != 112u) {
        std::printf("FAIL mem_write_override: default-off changed value\n");
        g_runtime_mem_write_override = nullptr;
        return false;
    }

    // Multiplier 1 is the faithful baseline; the game-owned override is
    // armed only for an enhanced 2x/3x player-speed mode.
    runtime_set_mem_write_override_enabled(2);
    codegen_test::bus_write_u32_direct(kAccumulator, 100u);
    runtime_trace_event(RUNTIME_TRACE_MEM_WRITE, kWriterPc, kAccumulator,
                        112u, 4u);
    bus_write_u32(kAccumulator, 112u);
    if (codegen_test::bus_read_u32_direct(kAccumulator) != 124u) {
        std::printf("FAIL mem_write_override: transform not applied\n");
        runtime_set_mem_write_override_enabled(0);
        g_runtime_mem_write_override = nullptr;
        return false;
    }

    // Wrong identity does not arm the store.
    codegen_test::bus_write_u32_direct(kAccumulator, 100u);
    runtime_trace_event(RUNTIME_TRACE_MEM_WRITE, kWriterPc + 2u,
                        kAccumulator, 112u, 4u);
    bus_write_u32(kAccumulator, 112u);
    if (codegen_test::bus_read_u32_direct(kAccumulator) != 112u) {
        std::printf("FAIL mem_write_override: wrong PC modified value\n");
        runtime_set_mem_write_override_enabled(0);
        g_runtime_mem_write_override = nullptr;
        return false;
    }

    // Any intervening write consumes the one-shot state; it must not leak.
    codegen_test::bus_write_u32_direct(kAccumulator, 100u);
    runtime_trace_event(RUNTIME_TRACE_MEM_WRITE, kWriterPc, kAccumulator,
                        112u, 4u);
    bus_write_u16(kAccumulator + 4u, 0xBEEFu);
    bus_write_u32(kAccumulator, 112u);
    if (codegen_test::bus_read_u32_direct(kAccumulator) != 112u) {
        std::printf("FAIL mem_write_override: stale state crossed write\n");
        runtime_set_mem_write_override_enabled(0);
        g_runtime_mem_write_override = nullptr;
        return false;
    }
...
int main() {
    ...
    if (!run_call_cancel_return_scan_cases()) ++failures;
    if (!run_thumb_alu_immediate_override_cases()) ++failures;
    if (!run_conditional_branch_override_cases()) ++failures;
    if (!run_infinite_cheat_cases()) ++failures;
    if (!run_player_speed_cheat_cases()) ++failures;
    if (failures) {
