...
// THUMB BL is architecturally two instructions (BL_prefix, BL_suffix). An
// IRQ taken between them resumes at the suffix halfword with the partial
// target already live in R14 (LR) from the prefix. gbarecomp must be able
// to enter generated code exactly at the suffix and reproduce the same
// state a full prefix+suffix sequential run would produce.
//
// This proves the mechanism at the interpreter level (the same semantics
// arm_codegen.cpp's BL_suffix emission relies on: it reads only g_cpu.R[14],
// so entering at the suffix statement is well-defined for the same reason).
//
//   BL_prefix at 0x100: 0xF000  (imm11 hi = 0)  -> LR = (0x100+4)+0 = 0x104
//   BL_suffix at 0x102: 0xF802  (imm11 lo = 2)  -> target = LR + (2<<1) = 0x108 (with fresh LR)
void test_thumb_bl_suffix_resume_matches_sequential() {
    // Run 1: sequential prefix+suffix from a clean state.
    auto cpu_seq = make_cpu();
    FlatBus bus_seq(64 * 1024);
    std::vector<uint16_t> seq_code = {
        0xF000u,  // BL_prefix, imm11 hi = 0
        0xF802u,  // BL_suffix, imm11 lo = 2
    };
    run_thumb(cpu_seq, bus_seq, 0x100u, seq_code, 4);

    // Run 2: an IRQ is taken after the prefix executes (LR is primed) but
    // before the suffix does. On resume, the runtime is entered directly at
    // the suffix halfword with LR already holding the value the prefix set.
    // Reproduce that: execute ONLY BL_prefix, snapshot LR, then start a
    // fresh interpreter step sequence at the suffix address with that LR.
    auto cpu_resume = make_cpu();
    FlatBus bus_resume(64 * 1024);
    bus_resume.write16(0x100u, 0xF000u);  // BL_prefix
    bus_resume.write16(0x102u, 0xF802u);  // BL_suffix
    cpu_resume.R[15] = 0x100u;
    cpu_resume.cpsr.t = true;
    cpu_resume.thumb = true;
    {
        auto i = armv4t::ThumbDecoder::decode(bus_resume.read16(0x100u), 0x100u);
        armv4t::Interpreter::step(cpu_resume, bus_resume, i);
    }
    // cpu_resume.R[15] is now 0x102 (the suffix) and R[14] holds the partial
    // target the prefix computed — exactly the state a post-IRQ resume at
    // the BL_suffix halfword observes. Simulate entry there directly (no
    // memory of having "just run" the prefix beyond the live LR value).
    {
        auto i = armv4t::ThumbDecoder::decode(
            bus_resume.read16(cpu_resume.R[15]), cpu_resume.R[15]);
        armv4t::Interpreter::step(cpu_resume, bus_resume, i);
    }

    check_eq_u32("thumb_bl_suffix_resume_matches_sequential", "r15",
                cpu_resume.R[15], cpu_seq.R[15]);
    check_eq_u32("thumb_bl_suffix_resume_matches_sequential", "r14",
                cpu_resume.R[14], cpu_seq.R[14]);
    check_eq_u32("thumb_bl_suffix_resume_matches_sequential",
                "expected_target", cpu_seq.R[15], 0x108u);
}
...
int main() {
    ...
    test_thumb_bl_suffix_resume_matches_sequential();
