...
bool parse_thumb_literal_overrides(
        const toml::array& arr,
        std::vector<ConfigThumbLiteralOverride>& out) {
    for (std::size_t i = 0; i < arr.size(); ++i) {
        const auto* t = arr[i].as_table();
        if (!t) {
            std::fprintf(stderr,
                "%s[[thumb_literal_override]] entry %zu is not a table\n",
                kAbortHeader, i);
            return false;
        }
        ConfigThumbLiteralOverride entry;
        bool ok = true;
        std::string err;
        entry.addr = get_u32_field(*t, "addr", true, ok, err);
        entry.note = get_string_field(*t, "note", false, ok, err);
        if (!ok || (entry.addr & 1u) != 0) {
            std::fprintf(stderr,
                "%s[[thumb_literal_override]] entry %zu: addr is "
                "required and must be halfword-aligned\n",
                kAbortHeader, i);
            return false;
        }
        for (const auto& prior : out) {
            if (prior.addr == entry.addr) {
                std::fprintf(stderr,
                    "%sduplicate [[thumb_literal_override]] at "
                    "0x%08X\n", kAbortHeader, entry.addr);
                return false;
            }
        }
...
bool parse_conditional_branch_overrides(
        const toml::array& arr,
        std::vector<ConfigConditionalBranchOverride>& out) {
    for (std::size_t i = 0; i < arr.size(); ++i) {
        const auto* t = arr[i].as_table();
        if (!t) {
            std::fprintf(stderr,
                "%s[[conditional_branch_override]] entry %zu is not a table\n",
                kAbortHeader, i);
            return false;
        }
        ConfigConditionalBranchOverride entry;
        bool ok = true;
        std::string err;
        entry.addr = get_u32_field(*t, "addr", true, ok, err);
        entry.note = get_string_field(*t, "note", false, ok, err);
        if (!ok || (entry.addr & 1u) != 0) {
            std::fprintf(stderr,
                "%s[[conditional_branch_override]] entry %zu: addr is "
                "required and must be halfword-aligned\n",
                kAbortHeader, i);
            return false;
        }
        for (const auto& prior : out) {
            if (prior.addr == entry.addr) {
                std::fprintf(stderr,
                    "%sduplicate [[conditional_branch_override]] at "
                    "0x%08X\n", kAbortHeader, entry.addr);
                return false;
            }
        }
...
bool parse_runtime_code_entries(
        const toml::array& arr,
        std::vector<ConfigRuntimeCodeEntry>& out) {
    for (std::size_t i = 0; i < arr.size(); ++i) {
        const auto* t = arr[i].as_table();
        if (!t) {
            std::fprintf(stderr,
                "%s[[runtime_code_entry]] entry %zu is not a table\n",
                kAbortHeader, i);
            return false;
        }
        ConfigRuntimeCodeEntry entry;
        bool ok = true;
        std::string err;
        entry.addr = get_u32_field(*t, "addr", true, ok, err);
        std::string mode_s = get_string_field(*t, "mode", true, ok, err);
        if (!ok || !parse_mode(mode_s, entry.mode)) {
            std::fprintf(stderr,
                "%s[[runtime_code_entry]] entry %zu: addr and mode "
                "(arm or thumb) are required\n", kAbortHeader, i);
            return false;
        }
        const uint32_t alignment =
            entry.mode == CpuMode::Thumb ? 2u : 4u;
        if ((entry.addr % alignment) != 0u) {
            std::fprintf(stderr,
                "%s[[runtime_code_entry]] entry %zu: 0x%08X is not "
                "%u-byte aligned\n", kAbortHeader, i, entry.addr, alignment);
            return false;
        }
        for (const auto& prior : out) {
            if (prior.addr == entry.addr && prior.mode == entry.mode) {
                std::fprintf(stderr,
                    "%sduplicate [[runtime_code_entry]] at 0x%08X/%s\n",
                    kAbortHeader, entry.addr, mode_s.c_str());
                return false;
            }
        }
        entry.note = get_string_field(*t, "note", false, ok, err);
...
bool validate_cross_section(const Config& cfg) {
    ...
    for (const auto& entry : cfg.runtime_code_entries) {
        ...
        bool in_code_image = entry.addr >= program_start &&
            entry.addr < program_end;
        for (const auto& cc : cfg.code_copies) {
            ...
            in_code_image = in_code_image ||
                (entry.addr >= copy_start && entry.addr < copy_end);
        }
        if (!in_code_image) {
            std::fprintf(stderr,
                "%s[[runtime_code_entry]] 0x%08X is outside the program "
        ...
        for (const auto& dr : cfg.data_ranges) {
            in_data_range = in_data_range ||
                (entry.addr >= dr.start && entry.addr < dr.end);
        }
        if (!in_data_range) {
            std::fprintf(stderr,
                "%s[[runtime_code_entry]] 0x%08X must begin inside a "
                "[[data_range]] containing the immutable placeholder bytes\n",
                kAbortHeader, entry.addr);
            return false;
        }
        for (const auto& ef : cfg.extra_funcs) {
            if (ef.addr == entry.addr) {
                std::fprintf(stderr,
                    "%s0x%08X is declared in BOTH [[runtime_code_entry]] "
                    "and [[extra_func]]\n", kAbortHeader, entry.addr);
                return false;
            }
        }
        for (const auto& ex : cfg.exclude_funcs) {
            if (ex.addr == entry.addr) {
                std::fprintf(stderr,
                    "%s0x%08X is declared in BOTH [[runtime_code_entry]] "
                    "and [[exclude_func]]\n", kAbortHeader, entry.addr);
                return false;
    ...
    for (const auto& lit : cfg.thumb_literal_overrides) {
        const std::uint64_t program_start = cfg.program.load_address;
        const std::uint64_t program_end =
            program_start + static_cast<std::uint64_t>(cfg.program.size);
        bool in_code_image = lit.addr >= program_start &&
            static_cast<std::uint64_t>(lit.addr) + 2u <= program_end;
        for (const auto& cc : cfg.code_copies) {
            const std::uint64_t copy_start = cc.runtime_start;
            const std::uint64_t copy_end =
                copy_start + static_cast<std::uint64_t>(cc.size);
            in_code_image = in_code_image ||
                (lit.addr >= copy_start &&
                 static_cast<std::uint64_t>(lit.addr) + 2u <= copy_end);
        }
        if (!in_code_image) {
            std::fprintf(stderr,
                "%s[[thumb_literal_override]] 0x%08X is outside "
                "the program image and declared [[code_copy]] spans\n",
                kAbortHeader, lit.addr);
            return false;
        }
        for (const auto& dr : cfg.data_ranges) {
            if (lit.addr >= dr.start && lit.addr < dr.end) {
                std::fprintf(stderr,
                    "%s[[thumb_literal_override]] 0x%08X falls inside "
                    "[[data_range]] [0x%08X,0x%08X)\n",
                    kAbortHeader, lit.addr, dr.start, dr.end);
                return false;
            }
        }
        for (const auto& ex : cfg.exclude_funcs) {
            if (ex.addr == lit.addr) {
                std::fprintf(stderr,
                    "%s[[thumb_literal_override]] 0x%08X is also "
                    "an [[exclude_func]] address\n", kAbortHeader, lit.addr);
                return false;
            }
        }
    }
    for (const auto& branch : cfg.conditional_branch_overrides) {
        const std::uint64_t program_start = cfg.program.load_address;
        const std::uint64_t program_end =
            program_start + static_cast<std::uint64_t>(cfg.program.size);
        bool in_code_image = branch.addr >= program_start &&
            static_cast<std::uint64_t>(branch.addr) + 2u <= program_end;
        for (const auto& cc : cfg.code_copies) {
            const std::uint64_t copy_start = cc.runtime_start;
            const std::uint64_t copy_end =
                copy_start + static_cast<std::uint64_t>(cc.size);
            in_code_image = in_code_image ||
                (branch.addr >= copy_start &&
                 static_cast<std::uint64_t>(branch.addr) + 2u <= copy_end);
        }
        if (!in_code_image) {
            std::fprintf(stderr,
                "%s[[conditional_branch_override]] 0x%08X is outside "
                "the program image and declared [[code_copy]] spans\n",
                kAbortHeader, branch.addr);
            return false;
        }
        for (const auto& dr : cfg.data_ranges) {
            if (branch.addr >= dr.start && branch.addr < dr.end) {
                std::fprintf(stderr,
                    "%s[[conditional_branch_override]] 0x%08X falls "
                    "inside [[data_range]] [0x%08X,0x%08X)\n",
                    kAbortHeader, branch.addr, dr.start, dr.end);
                return false;
            }
        }
        for (const auto& ex : cfg.exclude_funcs) {
            if (ex.addr == branch.addr) {
                std::fprintf(stderr,
                    "%s[[conditional_branch_override]] 0x%08X is also "
                    "an [[exclude_func]] address\n", kAbortHeader,
                    branch.addr);
                return false;
            }
...
bool load_config(const std::string& path, Config& out) {
    ...
    if (auto lit = tbl["thumb_literal_override"].as_array()) {
        if (!parse_thumb_literal_overrides(
                *lit, out.thumb_literal_overrides)) return false;
    }

    // [[conditional_branch_override]]
    if (auto branch = tbl["conditional_branch_override"].as_array()) {
        if (!parse_conditional_branch_overrides(
                *branch, out.conditional_branch_overrides)) return false;
    }
    ...
    // [[runtime_code_entry]]
    if (auto entry = tbl["runtime_code_entry"].as_array()) {
        if (!parse_runtime_code_entries(
                *entry, out.runtime_code_entries)) return false;
    }
...
void print_config_summary(const Config& cfg) {
    ...
    std::printf("  thumb literal hooks:   %zu\n",
                cfg.thumb_literal_overrides.size());
    std::printf("  conditional branch hooks: %zu\n",
                cfg.conditional_branch_overrides.size());
    std::printf("  ALU immediate hooks:   %zu\n",
                cfg.alu_immediate_overrides.size());
    std::printf("  data_range entries:    %zu\n", cfg.data_ranges.size());
    std::printf("  runtime code entries:  %zu\n",
