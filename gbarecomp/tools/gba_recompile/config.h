...
// Exact THUMB PC-relative literal load whose immutable word may consult a
// narrow game-owned runtime enhancement callback. This remains a memory load
// and keeps its normal bus timing; only the loaded value is replaceable.
struct ConfigThumbLiteralOverride {
    uint32_t    addr = 0;
    std::string note;
};

// Exact conditional-branch PC whose CPSR-derived decision may consult a
// game-owned runtime callback. The callback is deliberately opt-in per PC so
// ordinary generated code keeps the direct arm_cond_passes path.
struct ConfigConditionalBranchOverride {
    uint32_t addr = 0;
    std::string note;
};
...
// Exact entry into instructions synthesized in RAM at runtime. The immutable
// backing at addr remains a data_range; discovery ends at this boundary and
// runtime dispatch must select an identity-verified native image.
struct ConfigRuntimeCodeEntry {
    uint32_t    addr = 0;
    CpuMode     mode = CpuMode::Arm;
    std::string note;
};
...
struct Config {
    ...
    std::vector<ConfigThumbLiteralOverride> thumb_literal_overrides;
    std::vector<ConfigConditionalBranchOverride>
        conditional_branch_overrides;
    std::vector<ConfigAluImmediateOverride> alu_immediate_overrides;
    std::vector<ConfigDataRange>    data_ranges;
    std::vector<ConfigRuntimeCodeEntry> runtime_code_entries;
