...
addr = 0x03000828
mode = "arm"
note = "Runtime-overwritten instruction-template entry"
...
| `entry_pc`     | int hex | Initial PC. Seeded as ARM by default. An explicit `[[extra_func]]` at the same address supplies a reviewed THUMB mode/name when needed. |
...
### `[[thumb_literal_override]]` (zero or more)

Declares one reviewed, halfword-aligned THUMB PC-relative literal `LDR` whose
loaded word may consult the optional game-owned
`g_runtime_thumb_literal_override` callback. The normal bus read and timing
remain unchanged; only the loaded value is replaceable at the exact listed PC.
The callback's null/reject path preserves the original literal. Entries must
lie in the program image or a declared `[[code_copy]]` span and may not lie in
`[[data_range]]`.

### `[[conditional_branch_override]]` (zero or more)

Declares one reviewed, halfword-aligned conditional-branch instruction whose
original CPSR-derived decision may consult the optional game-owned
`g_runtime_conditional_branch_override` callback. Code generation emits the
chokepoint only at the exact listed PC; a null or rejecting callback preserves
the original decision. Entries must lie in the program image or a declared
`[[code_copy]]` span and may not lie in `[[data_range]]`. The declaration does
not alter the branch condition, target, timing, or any unlisted branch.
...
### `[[runtime_code_entry]]` (zero or more)

An exact, reviewed entry into instructions synthesized in RAM at runtime. This
is an external boundary for static discovery, not an assertion that the
immutable binary bytes are executable: the containing static function ends
with `runtime_dispatch(addr)`, and no function body is decoded at `addr`.

The entry must begin inside a declared `[[data_range]]`, so the immutable
placeholder or template bytes remain protected from static decoding. It must
also lie within the program image or a declared `[[code_copy]]` span. A
game-owned `RuntimeRamDispatchHook` can select an identity-verified AOT variant;
otherwise normal miss handling remains visible. In strict-static mode, an
unhandled entry is therefore still a loud dispatch miss and cannot be counted
as static coverage.

| key    | type    | required | meaning |
|--------|---------|----------|---------|
| `addr` | int hex | yes      | Exact runtime-produced instruction entry. |
| `mode` | string  | yes      | `"arm"` or `"thumb"`; controls alignment and dispatch identity. |
| `note` | string  | no       | Evidence/lifetime documentation. |

This declaration is intentionally narrow. It does not exempt other control
flow into a `data_range`, authorize decoding runtime placeholders, or describe
the generated image's identity. Those remain separate, game-owned evidence.
...
## Position-independent RAM images (`--relocatable-image`)

A `[[code_copy]]` declares that ROM bytes are executed at ONE runtime address.
Some games do not work that way: a flash driver keeps a working area in RAM and
copies whichever routine it needs into whatever base its allocator hands out,
and a routine DMA'd onto the stack lands wherever its caller's SP happens to be.
The same routine then executes at many bases, and the set is bounded only by the
call graph — so generating one corpus per (routine, base) is combinatorial and
never converges.

`gba_recompile --relocatable-image ORIGIN:SIZE` generates ONE corpus for such a
routine. Every guest address the image derives from its own PC — the PC pipeline
value, literal-pool bases, direct branch targets, link registers, resume and
trace PCs, `runtime_mem_cycles` region queries — is emitted as
`g_runtime_image_base + (addr - ORIGIN)` instead of a constant. `ORIGIN` is
just the address the corpus was generated at; nothing in the emitted code
depends on it.

This is a CLI flag rather than a TOML key on purpose: the config still describes
one concrete placement (a `[[code_copy]]` at `ORIGIN`, an `[[extra_func]]` entry
and a `[[resume_range]]` over the extent), and the flag changes only how that
placement is lowered. Reviewing the evidence and choosing to relocate stay
separate decisions.

What stays expressed in `ORIGIN`, because it never reaches the guest:

- `dispatch_table.cpp` addresses — the runtime subtracts the verified base;
- `goto` labels and generated temporary names;
- the resume tokens an alias wrapper writes to `g_runtime_resume_pc`;
- the config-key PC handed to `runtime_thumb_alu_immediate`.

Two extra symbols are emitted next to the dispatch table so a registration does
not have to restate them:

```c
extern "C" const uint32_t <prefix>kImageOrigin;
extern "C" const uint32_t <prefix>kImageSize;
```

The runtime contract is `g_runtime_image_base` (`src/armv4t/runtime_arm.h`). The
game's `RuntimeRamDispatchHook` sets it to a base it has VERIFIED — the live
bytes at `[base, base + SIZE)` must hash to the registered image identity —
immediately before calling the image's entry. Each generated image function
snapshots the global on entry and restores it before calling a sibling, so a
nested dispatch into a different image at a different base cannot corrupt the
caller's view. Identity, not address, remains what selects the code; relocation
only removes the requirement that the address be known in advance.
