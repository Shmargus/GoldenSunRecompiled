...
struct Bus {
    ...
    // Single-transfer data cost after any access side effects have committed.
    // Most buses use the ordinary N/S cost. Concrete systems with an
    // instruction-prefetch unit may adjust a below-cart data access according
    // to the currently executing code address (the GBA does this when WAITCNT
    // prefetch is enabled). A signed result is intentional: the GBA prefetch
    // calculation can hide more cycles than the raw data access contributed.
    virtual int32_t data_access_cycles(uint32_t addr, uint8_t width,
                                       bool sequential,
                                       uint32_t /*executing_pc*/) {
        return static_cast<int32_t>(access_cycles(addr, width, sequential));
    }
};
