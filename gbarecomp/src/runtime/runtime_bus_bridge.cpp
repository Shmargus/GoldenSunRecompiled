    ...
    while (g_host_profiling.load(std::memory_order_relaxed)) {
        ...
        if (h) {
            if (SuspendThread(h) != (DWORD)-1) {
                ...
                if (GetThreadContext(h, &ctx)) g_host_hist[ctx.Rip]++;
                ResumeThread(h);
            }
        }
...
static void tick_devices(gba::GbaBus* bus, gba::GbaPpu* ppu, uint32_t cycles) {
    ...
    while (remaining != 0) {
        ...
        if (events.vblank_started) {
            ...
            // mark_framebuffer_latched() only does pixel work (widescreen
            // margin rows) plus host-side presentation-readiness bookkeeping
            // (has_latched_fb_, diagnostics) — none of it guest-observable —
            // so it is safe to skip for a frame already discarded above.
            if (!g_skip_frame_render) ppu->mark_framebuffer_latched();
