...
// --scale, --screen, --volume, --linear-filter, --fullscreen,
...
inline void seam_append_setting_args(std::vector<std::string>& args,
    ...
    if (fixed_view_honored && opts.launcher_num_aspects > 0 &&
        ...
        const std::uint16_t raw_w = opts.launcher_aspect_view_widths[idx];
        const std::uint16_t raw_h = opts.launcher_aspect_view_heights
            ? opts.launcher_aspect_view_heights[idx] : 160;
        const std::uint16_t w = raw_w >= 240 ? raw_w : 240;
        const std::uint16_t h = raw_h >= 160 ? raw_h : 160;
        if (w > 240 || h > 160) {
            args.push_back("--view-width");
            args.push_back(std::to_string(w));
            if (h != 160) {
                args.push_back("--view-height");
        ...
        args.push_back("--resize-view");
               ...
               (opts.widescreen_view_width > 240 ||
                opts.widescreen_view_height > 160)) {
        args.push_back("--view-width");
        args.push_back(std::to_string(opts.widescreen_view_width >= 240
                                          ? opts.widescreen_view_width : 240));
        if (opts.widescreen_view_height > 160) {
            args.push_back("--view-height");
            args.push_back(std::to_string(opts.widescreen_view_height));
        }
    }
}
...
inline int gbarecomp_launcher_preboot(std::vector<std::string>& args,
    ...
    bool launcher_has_fixed_view = opts.widescreen_view_width > 240 ||
        opts.widescreen_view_height > 160;
    if (!launcher_has_fixed_view && opts.launcher_num_aspects > 1 &&
        opts.launcher_aspect_view_widths) {
        for (int i = 1; i < opts.launcher_num_aspects; ++i) {
