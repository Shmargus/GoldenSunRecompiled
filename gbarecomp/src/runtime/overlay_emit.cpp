...
#include "arm_codegen.h"      // RelocatableImage
...
std::string emit_overlay_c(uint32_t pc, bool thumb,
    ...
    image.origin = pc;
    image.size = target->end_addr - pc;
    std::string body = emit_function_body_str(
        *target, bytes, size, base, empty_names, nullptr,
        relocatable ? &image : nullptr);
