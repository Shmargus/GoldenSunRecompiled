...
#define g_runtime_image_base (*g_ovl->runtime_image_base)
...
static inline int runtime_ram_code_guard(uint32_t start, uint32_t end,
                                         uint32_t expected_crc32) {
    return g_ovl->runtime_ram_code_guard(start, end, expected_crc32);
}
static inline void runtime_mutable_ram_code_miss(uint32_t pc, int thumb) {
    g_ovl->runtime_mutable_ram_code_miss(pc, thumb);
}
...
static inline void runtime_idle_backedge(uint32_t header_pc) { g_ovl->runtime_idle_backedge(header_pc); }
