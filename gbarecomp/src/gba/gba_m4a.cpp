...
void mp2k_dump_live(GbaBus& bus, std::string& out) {
    ...
    for (int ch = 0; ch < kMp2kMaxChans; ++ch) {
        ...
        appendf("%s{\"ch\":%d,\"active\":%s,\"status\":%u,\"type\":%u,"
                ...
                static_cast<unsigned>(count), static_cast<unsigned>(fw),
                static_cast<unsigned>(freq), static_cast<unsigned>(wav),
                static_cast<unsigned>(cp), wav_ok ? "true" : "false",
