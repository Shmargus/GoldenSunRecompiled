...
namespace gba {
namespace {
constexpr uint32_t kMeasuredRamImageStart = 0x03005B0Cu;
constexpr uint32_t kMeasuredRamImageEnd = 0x03005D5Cu;
...
uint32_t measured_ram_image_crc32(armv4t::Bus& bus) {
    std::array<uint8_t, kMeasuredRamImageEnd - kMeasuredRamImageStart> bytes{};
    for (std::size_t i = 0; i < bytes.size(); ++i) {
        bytes[i] = bus.read8(kMeasuredRamImageStart + static_cast<uint32_t>(i));
    }
    return crc32(bytes.data(), bytes.size());
}

// Return true when at least one destination unit of a DMA block touches the
// measured image.  The old probe required one exact incrementing 592-byte
// transfer; real hardware may fill the same image in split blocks, or use a
// decrement/fixed destination mode.  This is address-only metadata: no DMA
// payload is retained or emitted.
bool dma_destination_overlaps_image(uint32_t dest, uint32_t step,
                                    uint32_t word_count,
                                    uint32_t dest_ctrl) {
    uint64_t first = dest;
    uint64_t last_exclusive = dest;
    const uint64_t span = static_cast<uint64_t>(step) * word_count;
    if (dest_ctrl == 1u) {  // decrement
        last_exclusive = static_cast<uint64_t>(dest) + step;
        first = last_exclusive - span;
    } else if (dest_ctrl == 2u) {  // fixed
        last_exclusive = static_cast<uint64_t>(dest) + step;
    } else {  // increment and increment+reload during this block
        last_exclusive = static_cast<uint64_t>(dest) + span;
    }
...
void GbaIo::run_immediate_dma(int channel) {
    ...
    uint32_t s = sad;
    uint32_t d = dad;
    const uint32_t transfer_size = word_count * step;
    ...
    if (measured_image_transfer) {
        ...
        runtime_note_ram_image_dma(
            1u, sad, dad, transfer_size,
            measured_ram_image_crc32(*bus_), static_cast<uint32_t>(channel));
    }
    vram_trace::end_dma();
    vram_trace::trace_oam_shadow_dma(
        bus_, channel, runtime_current_pc(), sad, dad, transfer_size, cnt_h, 0);
...
void GbaIo::run_timed_dma(int start_mode) {
    ...
    for (int channel = 0; channel < 4; ++channel) {
        ...
        const uint32_t dma_source_start = s;
        const uint32_t dma_dest_start = d;
        vram_trace::trace_dma(io_.data(), channel, runtime_current_pc(),
        ...
        if (measured_image_transfer) {
            runtime_note_ram_image_dma(
                0u, dma_source_start, dma_dest_start, word_count * step, 0u,
                static_cast<uint32_t>(channel));
            runtime_ram_image_dma_begin();
        }
        ...
        if (measured_image_transfer) {
            runtime_ram_image_dma_end();
            runtime_note_ram_image_dma(
                1u, dma_source_start, dma_dest_start, word_count * step,
                measured_ram_image_crc32(*bus_),
                static_cast<uint32_t>(channel));
        }
        vram_trace::end_dma();
        vram_trace::trace_oam_shadow_dma(
            bus_, channel, runtime_current_pc(), dma_source_start, dma_dest_start,
            word_count * step, cnt_h, start_mode);
        ++dma_runs_[channel];
        dma_words_[channel] += word_count;
...
void GbaIo::run_sound_fifo_dma(int channel) {
    ...
    if (std::getenv("GBARECOMP_AUDIO_PROBE")) {
        static uint32_t probe_dma = 0;
        if (probe_dma++ < 32) {
            std::fprintf(stderr,
                         "[audio-dma] ch=%d src=0x%08x dst=0x%08x ctrl=0x%04x\n",
                         channel, sad, dad, cnt_h);
        }
    }
    ...
    for (int i = 0; i < 4; ++i) {
        if (audio_)
            audio_->sound_fifo_dma_word(channel - 1, s, g_runtime_cycles);
        const uint32_t word = bus_->read32(s);
        if (audio_)
            audio_->set_sound_fifo_dma_write_context(true);
        bus_->write32(dad, word);
        if (audio_) {
            audio_->set_sound_fifo_dma_write_context(false);
            audio_->capture_sound_fifo_dma_word(
                channel - 1, s, word, g_runtime_cycles);
        }
        switch (src_ctrl) {
            case 0: s += 4; break;
            case 1: s -= 4; break;
    ...
    for (int t = 0; t < 4; ++t) {
        ...
        while (ticks != 0) {
            ...
            if (audio_ && (t == 0 || t == 1)) {
                ...
                audio_->timer_overflow(t, g_runtime_cycles);
                if (refill_a) {
...
uint16_t GbaIo::waitcnt()  const { return load_u16(&io_[IoReg::WAITCNT]); }
...
void GbaIo::notify_affine_reference_write(uint32_t off, uint32_t size) {
    if (!ppu_ || size == 0) return;
    const uint32_t end = off + size;
    for (uint32_t bg = 0; bg < 2; ++bg) {
        const uint32_t xoff = 0x28u + bg * 0x10u;
        const uint32_t yoff = xoff + 4u;
        if (off < xoff + 4u && end > xoff)
            ppu_->reload_affine_reference(bg + 2u, false,
                                          load_u32(&io_[xoff]));
        if (off < yoff + 4u && end > yoff)
            ppu_->reload_affine_reference(bg + 2u, true,
                                          load_u32(&io_[yoff]));
    }
}
...
void GbaIo::write8(uint32_t off, uint8_t v) {
    ...
    if (audio_ && off >= 0x060 && off <= 0x0AF) {
        audio_->write_io8(off, v, g_runtime_cycles);
    }
    switch (off) {
        case IoReg::HALTCNT:
    ...
    io_[off] = v;
    notify_affine_reference_write(off, 1);
}
...
void GbaIo::write16(uint32_t off, uint16_t v) {
    ...
    store_u16(&io_[off], v);
    notify_affine_reference_write(off, 2);
}
...
void GbaIo::write32(uint32_t off, uint32_t v) {
    ...
    if (audio_ && off >= 0x060 && off <= 0x0AF) {
        audio_->write_io32(off, v, g_runtime_cycles);
        if (off == 0x080) {
